...
def getQuoteListOption(
    yyyymm_weekth: str, premium_upper_bound: int, premium_lower_bound: int
) -> pd.DataFrame:
    market_time = datetime.now().strftime("%H:%M")
    if not (
        "08:45" < market_time < "13:45"
        or market_time > "15:00"
        or market_time < "05:00"
    ):
        return print("請在台指期交易時間內使用")
    arg1, arg2, market_type = (
        ("TXFI5-F", "TXO-Q", "0")
        if "08:45" < market_time < "13:45"
        else ("TXFI5-M", "TXO-R", "1")
    )
    last_close = float(
        requests.post(
            "https://mis.taifex.com.tw/futures/api/getQuoteDetail",
            json={"SymbolID": ["TXF-S", arg1, arg2]},  #! 這裡的 TXF-S 是台指期當月契約
        ).json()["RtData"]["QuoteList"][1]["CLastPrice"]
    )
    strike_range = [str(int(last_close / 50) * 50 + i * 50) for i in range(-3, 5)]
    quote_table = requests.post(
        "https://mis.taifex.com.tw/futures/api/getQuoteListOption",
        json={
    ...
    data = {
        item["DispEName"][-6:]: (
            float(item["CBestAskPrice"]) + float(item["CBestBidPrice"])
        )
        / 2
        for item in quote_table
        if item["DispEName"][-6:-1] in strike_range
    }
    call_data, put_data = {k: v for k, v in data.items() if k.endswith("C")}, {
        k: v for k, v in data.items() if k.endswith("P")
    }
    strikes = sorted(set(k[:5] for k in call_data.keys()), reverse=True)
    df = pd.DataFrame(
        {
            "call": [call_data.get(s + "C", 0) for s in strikes],
            "put": [put_data.get(s + "P", 0) for s in strikes],
        },
        index=strikes,
    ).astype(object)
...
def amount_rank(chg_pct=1, vola_pct=4, twse_weight=50):
    df = pd.read_html("https://histock.tw/stock/rank.aspx?m=13&d=1&p=all")[0][
        [
            "代號▼",
            "名稱▼",
            "漲跌幅▼",
